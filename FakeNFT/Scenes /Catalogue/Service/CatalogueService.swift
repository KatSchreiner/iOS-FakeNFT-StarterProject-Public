...
final class CatalogueService: CatalogueServiceProtocol {
    ...
    func fetchCollections(completion: @escaping (Result<[NFTCollections], Error>) -> Void) {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            self.task = networkClient.send(request: request, type: [NFTCollections].self) { result in
                self.task = nil
    ...
    func updateCart(with nft: String, in cart: [String], completion: @escaping (Result<CatalogueCart, Error>) -> Void) {
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            let bodyString = cart.map { "nfts=\($0)" }.joined(separator: "&")
            guard let requestData = bodyString.data(using: .utf8) else {
                completion(.failure(NSError(domain: "Encoding error", code: -1)))
                return
            }
            
            let request = UpdateCartRequest(endpoint: url, httpMethod: .put, body: requestData, contentType: "application/x-www-form-urlencoded")
    ...
    func updateLike(with nft: String, in likes: [String], completion: @escaping (Result<UserProfile, Error>) -> Void) {
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            let bodyString = likes.map { "likes=\($0)" }.joined(separator: "&")
            guard let requestData = bodyString.data(using: .utf8) else {
                completion(.failure(NSError(domain: "Encoding error", code: -1)))
                return
            }
